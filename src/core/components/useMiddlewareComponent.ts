...
import {flattenConfig, PluginOptions, useSource} from 'sanity'
...
/** @internal */
export function useMiddlewareComponent<T extends {}>(props: {
  ...
  return useMemo(() => {
    ...
    const result = pickedComponents.filter(Boolean) as ComponentType<T>[]
